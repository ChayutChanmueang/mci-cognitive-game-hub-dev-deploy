...
**Status:** 🏗 In-Progress (Sprint 7) — AC#1–5 เสร็จ (v0.15.0, ยืนยันบนมือถือ 2026-06-26); **ขยายขอบเขต 2026-06-26:** เพิ่ม AC#6 เปลี่ยน emoji บน popup จบโปรแกรมเป็นคุณตา/คุณยาย (รอทำ)
...
4. [x] ตัวละครคนแก่ใน Popup มี **เงาด้านล่าง** ตาม [US-E7-14](US-E7-14.md) ข้อ 4 — ใช้ shadow มาตรฐาน `.character-shadow` (Figma `Character_Shadow` 3165:775, วงรีเทา `rgba(121,121,121,0.34)` ไม่ blur) — v0.16.0 ทำเป็น component กลางใช้ซ้ำทุกที่ที่มีรูปคุณตา/ยาย
5. [x] ทดสอบ flow จริงบนมือถือ *(ยืนยันแสดงผลถูกต้อง 2026-06-26)*
6. [ ] **Popup "ยินดีด้วย คุณเล่นจบโปรแกรมทั้งหมดจนครบ 14 วันแล้ว"** (`showProgramCompletionPopup`) — เปลี่ยน emoji 🧓 ให้เป็น **รูปคุณตา/คุณยายตามเพศ** (`OldMan_*`/`OldWoman_*`) และจัด **layout คล้าย popup "วันนี้พักก่อน"** (ตัวละคร + เงาด้านล่าง + ปุ่มเขียว) — *เพิ่มขอบเขต 2026-06-26*
...
- [ ] **(AC#6)** ปรับ `showProgramCompletionPopup` ให้ใช้ตัวละครคุณตา/คุณยายตามเพศแทน emoji 🧓 + จัด layout แบบเดียวกับ `showDayCompletionPopup` (ส่ง `gender` เข้ามาจากจุดเรียกใน `game-hub-screen.js`) + ใส่ shadow มาตรฐาน `.character-shadow` ใต้ตัวละคร; พิจารณาเพิ่มปุ่มทดสอบใน FAB menu ด้วย
